...
// h2Refs 요소들에 대해 IntersectionObserver 적용
useEffect(() => {
...
// data-io 요소들을 감지하여 애니메이션 적용
useEffect(() => {
  const ioElements = document.querySelectorAll('[data-io]');
  
  if (ioElements.length > 0) {
    ioElementsRef.current = ioElements; // 요소들을 ioElementsRef에 저장
        ...
        <Carousel activeIndex={index} onSelect={handleSelect}>
          <Carousel.Item>
            ...
            id='img_size'
              ...
              src={main01}
              alt="너는 맛이 참 곱구마 빠다 고구마 모두가 반한 그 맛 단짠단짠 맛있구마 with 타로폼"
            />
          </Carousel.Item>
          <Carousel.Item>
            <img
              className="d-block w-100"
              src={main02}
              alt="이달의 신메뉴 빠다 고구마 4종 구매 시 스탬프 3배 적립 2024년 9월 3일부터 7일까지 7일간"
            />
          </Carousel.Item>
          <Carousel.Item>
            <img
          ...
          <Carousel.Item>
            <img
              className="d-block w-100"
              src={main05}
              alt="돌아온 초코바른 당신이 찾던 그때 그 맛 그대로"
            />
          </Carousel.Item>
          <Carousel.Item>
            <img
              className="d-block w-100"
              src={main06}
              alt="LG U+와 공차 콜라보 u+멤버십 VIP콕 회원 공차 블랙밀크티 무료 증정"
            />
          </Carousel.Item>
          <Carousel.Item>
            <img
              className="d-block w-100"
              src={main07}
              alt="베스트셀러 1위 블랙 밀크티+펄 2위 초콜릿 밀크티+밀크폼 3위 망고요구르트+화이트펄 4위 딸기 쿠키 스무디+펄 5위 우롱티+코코넛+밀크폼"
            />
          </Carousel.Item>
        </Carousel>
    </div>
      ...
      <h2 ref={(el) => (h2Refs.current[1] = el)} className='slide-up'>Tea receipe</h2>
        ...
        {selectedTea === 'tea_01' && (
          <div className='cont_01'>
            ...
            <dl>
              <dt>Original tea</dt>
              <dd>티백이나 가루가 아닌 황실에 바치던 잎차만 골라 1도 1초도 어기지 않고<br />
              다른 황금률로 우려 한 잔 한 잔 부드러움을 더해 만듭니다.
              </dd>
            </dl>
          </div>)}
          ...
          <div className= 'cont_02'>
            ...
            <dl>
              <dt>Milk tea</dt>
              <dd>최적의 자연이 키워낸 입차만 골라 각 차종에 맞는 황금률로 우려 담백한 우유와 <br />
          ...
          <div className='cont_03'>
            ...
            <dl>
              <dt>Fruit tea</dt>
              <dd>찻잎의 푸름을 간직한 녹차만 골라 최적의 온도 89℃로 우려<br />
              달콤상큼한 과일을 더해 만듭니다.
              </dd>
            </dl>
          </div>)}
